...
Licensed under the MIT License — Copyright (c) TANGandXue and contributors.
The full MIT license text is included in this repository's LICENSE file.
"""
...
def _verify_windows_direct_key_candidate(
    ...
    if not remaining_salts:
        return 0
    try:
        passphrase = bytes.fromhex(enc_key_hex)
    except ValueError:
        return 0
    if len(passphrase) != KEY_SZ:
        return 0
    matched = 0
    # 微信 4.1+: Config.Cipher blob 里是 passphrase，需 PBKDF2 派生后才可验证
    for salt_hex in list(remaining_salts):
        salt = bytes.fromhex(salt_hex)
        derived = hashlib.pbkdf2_hmac("sha512", passphrase, salt, 256000, dklen=KEY_SZ)
        for _rel, _path, _sz, s, page1 in db_files:
            if s == salt_hex and verify_enc_key(derived, page1):
                key_map[salt_hex] = derived.hex()
