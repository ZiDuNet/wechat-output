# wechat-group-export · 微信群聊导出（Windows）

从微信 Windows 4.x 的本地加密数据库提取密钥、解密，把指定群聊导出为 Markdown 的一键工具链。**零第三方依赖即可完成提取+解密**；解压富文本消息需 zstandard（1.8MB）。
...
# 2. 顺带产出统计底座库（wechat-group-digest 的前置数据）
...
- 缓存（`~/.wxcache`）内含**数据库密钥与明文聊天记录**，属敏感数据：用完可 `--purge` 删除，或自行妥善保管
